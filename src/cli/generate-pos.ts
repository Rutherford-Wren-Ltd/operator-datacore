        ...
        for (const [supplierId, skus] of suppliers) {
          ...
          const { rows: poRows } = await pg.query<{ po_id: string }>(
            ...
            [poNumber, supplierId, supplierCcy, syncRunId,
              ...
              + `${skus.length} line(s). Review, assign destinations, allocate a real PO `
              + 'number, then promote out of draft.'],
          );
            ...
            await pg.query(
              `INSERT INTO brain.purchase_order_lines
               ...
               VALUES ($1, $2, 'product', $3, $4, $5,
                       'unallocated', 'unallocated', $6, 0, 'open', $7)`,
              [poId, lineNo, r.ean, r.asin, r.title.slice(0, 200),
                r.proposedQty, lineNote(r)],
            );
            linesWritten += 1;
          }
        }
...
function lineNote(r: SkuResult): string {
  ...
  return `UK/EU gap ${fmt(r.gapUkEu)} (demand ${fmt(r.demandUkEu)} / supply ${fmt(r.supplyUkEu)}); `
    + `NA gap ${fmt(r.gapNa)} (demand ${fmt(r.demandNa)} / supply ${fmt(r.supplyNa)}); `
    + `global committed ${fmt(r.committedGlobal)}; net ${fmt(r.netGap)} -> ${fmt(r.proposedQty)} `
    + `(MOQ ${fmt(r.moq)}). Demand: ${basis}. `
    + `Velocity ${r.velocity30d.toFixed(1)}/day, trend ${r.trend}. ${margin}.`
...
function buildReport(d: ReportInput): string {
  ...
  L.push('Generated by the restock engine (`npm run generate-pos`). These are **draft POs '
    + 'for review** — not placed orders. Assign physical destinations, allocate a real PO '
  ...
  L.push('2. For a PO you approve: in `brain.purchase_orders`, assign each line a real '
    + '`destination` / `serves_region` (it is `unallocated`), rename `po_number` to the '
    + 'real PO number, then promote it with `npm run set-po-status`.');
