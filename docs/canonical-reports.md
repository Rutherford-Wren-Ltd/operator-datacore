...
| Fees (referral, FBA, storage, etc.) | `Finances API → listFinancialEvents` | `brain.financial_events` | Event | 24-48h | **active** (CLI: `npm run ingest-finances`). Parses ShipmentEvent, RefundEvent, ServiceFeeEvent, AdjustmentEvent, ProductAdsPaymentEvent in v1; other event types land raw only |
| Refunds (financial side) | `Finances API → listFinancialEvents` (RefundEvent type) | `brain.refund_events` (view) | Event | 24-48h | **active** (lands via `npm run ingest-finances`; the view joins to brain.orders by amazon_order_id) |
...
| Search query performance | `GET_BRAND_ANALYTICS_SEARCH_QUERY_PERFORMANCE_REPORT` | `brain.search_query_performance` | Period × ASIN × query | 1 week | **active** (scheduled via `.github/workflows/sqp-sync.yml`: WEEK weekly, MONTH monthly, top-N ASINs; CLI `npm run backfill-search-query` for history / wider scope) |
...
| Sponsored Brands spend + attributed sales | **Amazon Ads API v3** → `sbPurchasedProduct` + `sbCampaigns` | `brain.ads_sb_daily` | Day × campaign × ad group × purchased ASIN (sales) + Day × campaign (cost) | 24-48h | **active (opt-in)** (CLI: `npm run ads-ingest -- --products SP,SD,SB`). Two reports fire in parallel: per-ASIN sales rows + campaign-level cost rows; analytics views JOIN by (date, profile, campaign) to spread cost across ASINs by sales share |
