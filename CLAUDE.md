...
1. **Brand Analytics (SQP) coverage is top-N, not universal.** `brain.search_query_performance` is now pulled on a schedule (`.github/workflows/sqp-sync.yml`: WEEK weekly on Wednesdays, MONTH monthly on the 8th), but only for the **top-N ASINs by recent units per marketplace**, plus whatever has been hand-backfilled. ASINs outside that scope, or newly launched, can still return zero rows, and zero rows means "not in scope / never backfilled", not "no search traffic". Before reporting on a SKU's keyword funnel, confirm coverage for that ASIN and period; if empty, run the backfill scoped to that ASIN (UK and US separately) per [docs/runbooks/search-query-backfill.md](./docs/runbooks/search-query-backfill.md). Also mind the Amazon publish lag: WEEK data lands a few days after the Sun-Sat week closes and MONTH a few days into the next month, so the freshest complete period is not yesterday.
